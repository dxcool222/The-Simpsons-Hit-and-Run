...
void ZoneEventLocator::SetZone( const char* zone )
{
    ...
    const unsigned int requiredSize = (unsigned int)zoneStrLength + 1;
    if( mZone == NULL || mZoneSize < requiredSize )
    {
        if( mZone )
        {
            delete[] mZone;
        ...
        mZoneSize = requiredSize;
        mZone = new(GMA_LEVEL_OTHER) char[mZoneSize];
    }

    strncpy( mZone, zone, mZoneSize - 1 );
    mZone[mZoneSize - 1] = '\0';
