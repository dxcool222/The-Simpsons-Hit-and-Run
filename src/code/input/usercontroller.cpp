...
void UserController::OnControllerInputPointChange( unsigned int buttonId, float value )
{
...
#if defined(RAD_TVOS_INPUT_DIAGNOSTICS)
    {
        ...
        // Avoid spamming logs for stick axes (they sit around 0.5 and change constantly).
        const bool isAxis = ( type != NULL ) && ( ::strcmp( type, "XAxis" ) == 0 || ::strcmp( type, "YAxis" ) == 0 );
        if( !isAxis )
        {
            if ( ( fLastValue > 0.5f ) || ( value > 0.5f ) )
            {
                ...
                printf( "[UserController] id=%d buttonId=%u name='%s' %.2f -> %.2f\n", m_controllerId, buttonId, name, fLastValue, value );
                fflush( stdout );
            }
        }
