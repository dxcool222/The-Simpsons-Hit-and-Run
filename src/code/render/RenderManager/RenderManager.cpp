...
//========================================
#include <stdio.h>
#include <string.h>  // for strstr in diagnostic logging
...
#if ENABLE_DYNA_AUDIT
#warning "DYNA AUDIT ENABLED"
#endif
...
static bool IsAuditFile(const char* s) {
    if(!gAuditDyna || !s) return false;
    return StrIContains(s,"l1r2.p3d") || StrIContains(s,"l1z3.p3d") ||
           StrIContains(s,"l1r3.p3d") || StrIContains(s,"l1_terra.p3d");
}

// Tracking for one-time-per-file logs
static bool gSeenAddReq_l1r2 = false;
static bool gSeenAddReq_l1z3 = false;
static bool gSeenAddReq_l1r3 = false;
static bool gSeenAddReq_terra = false;

static bool SeenAddReq(const char* s) {
    if(StrIContains(s,"l1r2.p3d"))      { if(gSeenAddReq_l1r2)  return true; gSeenAddReq_l1r2  = true; return false; }
    if(StrIContains(s,"l1z3.p3d"))      { if(gSeenAddReq_l1z3)  return true; gSeenAddReq_l1z3  = true; return false; }
    if(StrIContains(s,"l1r3.p3d"))      { if(gSeenAddReq_l1r3)  return true; gSeenAddReq_l1r3  = true; return false; }
    if(StrIContains(s,"l1_terra.p3d"))  { if(gSeenAddReq_terra) return true; gSeenAddReq_terra = true; return false; }
    return true;
}

// Per-file chunk counters (Phase 2 + 5)
const char* gAuditCurrentFile = NULL;
static int gAuditCnt_RoadSeg = 0;
static int gAuditCnt_Road = 0;
static int gAuditCnt_Intersect = 0;
static int gAuditCnt_StaticEntity = 0;
static int gAuditCnt_StaticPhys = 0;
static int gAuditCnt_IntersectDSG = 0;
static int gAuditCnt_DroppedBogus = 0;
static int gAuditCnt_DroppedIgnore = 0;

// AddGuts tracking (Phase 3)
int gAuditAddGuts_RoadSeg = 0;
int gAuditAddGuts_SpatialNull = 0;
int gAuditAddGuts_StateLoad = 0;
int gAuditAddGuts_StatePre = 0;
int gAuditAddGuts_StateNoLoad = 0;
int gAuditAddGuts_StaticEntity = 0;
int gAuditAddGuts_StaticPhys = 0;
int gAuditAddGuts_Intersect = 0;
int gAuditAddGuts_Reject_InvalidLayer = 0;
int gAuditAddGuts_Reject_FilteredType = 0;

static void AuditResetCounters() {
    gAuditCnt_RoadSeg = 0;
    gAuditCnt_Road = 0;
    gAuditCnt_Intersect = 0;
    gAuditCnt_StaticEntity = 0;
    gAuditCnt_StaticPhys = 0;
    gAuditCnt_IntersectDSG = 0;
    gAuditCnt_DroppedBogus = 0;
    gAuditCnt_DroppedIgnore = 0;
    gAuditAddGuts_RoadSeg = 0;
    gAuditAddGuts_SpatialNull = 0;
    gAuditAddGuts_StateLoad = 0;
    gAuditAddGuts_StatePre = 0;
    gAuditAddGuts_StateNoLoad = 0;
    gAuditAddGuts_StaticEntity = 0;
    gAuditAddGuts_StaticPhys = 0;
    gAuditAddGuts_Intersect = 0;
    gAuditAddGuts_Reject_InvalidLayer = 0;
    gAuditAddGuts_Reject_FilteredType = 0;
}

static void AuditPrintParseDone() {
    if( gAuditDyna && gAuditCurrentFile && IsAuditFile( gAuditCurrentFile ) )
    {
        printf(
            "[AUDIT][SUMMARY] file=%s roadSeg=%d road=%d inter=%d static=%d phys=%d "
            "intersectDSG=%d droppedBogus=%d droppedIgnore=%d\n",
            gAuditCurrentFile,
            gAuditCnt_RoadSeg,
            gAuditCnt_Road,
            gAuditCnt_Intersect,
            gAuditCnt_StaticEntity,
            gAuditCnt_StaticPhys,
            gAuditCnt_IntersectDSG,
            gAuditCnt_DroppedBogus,
            gAuditCnt_DroppedIgnore );
        fflush( stdout );

        printf(
            "[AUDIT][ADDED] file=%s addRoadSeg=%d addStatic=%d addPhys=%d addIntersect=%d "
            "roadSpatialNull=%d roadStateLoad=%d roadStatePre=%d roadStateNoLoad=%d "
            "rejectInvalidLayer=%d rejectFilteredType=%d\n",
            gAuditCurrentFile,
            gAuditAddGuts_RoadSeg,
            gAuditAddGuts_StaticEntity,
            gAuditAddGuts_StaticPhys,
            gAuditAddGuts_Intersect,
            gAuditAddGuts_SpatialNull,
            gAuditAddGuts_StateLoad,
            gAuditAddGuts_StatePre,
            gAuditAddGuts_StateNoLoad,
            gAuditAddGuts_Reject_InvalidLayer,
            gAuditAddGuts_Reject_FilteredType );
        fflush( stdout );
    }
    gAuditCurrentFile = NULL;
    AuditResetCounters();
}
//==============================================================================
...
static inline bool IsTimelineZone(const char* name) {
    ...
    return (strstr(name, "l1z3") || strstr(name, "L1Z3") ||
            strstr(name, "l1z1") || strstr(name, "L1Z1"));
}
static inline const char* GetTimelineTag(const char* name) {
    if (!name) return "???";
    if (strstr(name, "l1z3") || strstr(name, "L1Z3")) return "L1Z3";
    if (strstr(name, "l1z1") || strstr(name, "L1Z1")) return "L1Z1";
    return "???";
}
#endif
//==============================================================================
...
{ 
   ...
   {
      once = true;
      printf("AUDIT_BOOT: OnChunkLoaded reached\n");
      fflush( stdout );
   }
   #endif

   // PHASE 5: Count drops due to BogusUserData/IgnoreGuts (only for audit files)
   ...
   {
      if(gAuditDyna && gAuditCurrentFile) {
         if(iUserData == RenderEnums::BogusUserData) gAuditCnt_DroppedBogus++;
         else gAuditCnt_DroppedIgnore++;
      }
      return;
   }
  ...
  // }
   ...
   switch( iChunkID ) 
   {
    ...
    case SRR2::ChunkID::ENTITY_DSG:
      // PHASE 2: Count static entities for audit files
      if(gAuditDyna && gAuditCurrentFile) gAuditCnt_StaticEntity++;
   ...
   case SRR2::ChunkID::STATIC_PHYS_DSG:
      // PHASE 2: Count static phys for audit files
      if(gAuditDyna && gAuditCurrentFile) gAuditCnt_StaticPhys++;
      switch( iUserData & RenderEnums::GutsOnlyMask )
      {
   ...
   case SRR2::ChunkID::INTERSECT_DSG:
      // PHASE 2: Count intersect DSG for audit files
      if(gAuditDyna && gAuditCurrentFile) gAuditCnt_IntersectDSG++;
      switch( iUserData & RenderEnums::GutsOnlyMask )
      {
...
//              mpRenderLayers[iUserData & RenderEnums::LayerOnlyMask]->AddGuts((AnimEntityDSG*)ipEntity);
   ...
   case Pure3D::Mesh::MESH:
      ...
      case RenderEnums::IgnoreGuts:
         // PHASE C: Count filtered-out mesh chunks for audit files
         if( gAuditDyna && gAuditCurrentFile )
         {
            gAuditAddGuts_Reject_FilteredType++;
         }
   ...
   case SRR2::ChunkID::ROAD_SEGMENT:
      // PHASE 2: Count road segments for audit files
      if(gAuditDyna && gAuditCurrentFile) gAuditCnt_RoadSeg++;
      switch( iUserData & RenderEnums::GutsOnlyMask )
      {
    ...
    if( ((*(int*)pUserData) & RenderEnums::CompletionOnlyMask) == RenderEnums::DynamicLoadComplete )
    {
...
#if defined(RAD_TVOS) && RAD_TVOS_ZONE_PIPELINE_LOG
        {
            ...
            if (IsTimelineZone(zoneName)) {
                const char* tag = GetTimelineTag(zoneName);
                printf("[TL:%s] F%u STAGE3_CALLBACK_ENTRY zoneName='%s' zoneIdx=%d pUserData=0x%08X\n",
                    tag, gTimelineFrame, zoneName, i, *(int*)pUserData);
                printf("[TL:%s] F%u STAGE3_CALLBACK_CONTEXT mbDynaLoading=%d mbLoadZonesDumped=%d numLoadZones=%d\n",
                    tag, gTimelineFrame, mbDynaLoading ? 1 : 0, mbLoadZonesDumped ? 1 : 0, mpZEL->GetNumLoadZones());
                fflush(stdout);
            }
        }
#endif

        // PHASE 2: Print ParseDone summary BEFORE DoPostDynaLoad clears state
        AuditPrintParseDone();
        ...
        {
            // PHASE C: Per-file state snapshot for audited dynamic zones
            const char* zoneName = mpZEL->GetLoadZone( i );
            if( gAuditDyna && zoneName && IsAuditFile( zoneName ) )
            {
                bool worldLayerValid = ( mCurWorldLayer >= 0 &&
                                         mCurWorldLayer < RenderEnums::numLayers &&
                                         mpRenderLayers[mCurWorldLayer] != NULL );
                printf(
                    "[AUDIT][STATE] file=%s level=%d layerBits=0x%08X curWorldLayer=%d "
                    "worldLayerValid=%d loadZonesDumped=%d queuedZELs=%d\n",
                    zoneName,
                    msLevel,
                    (*(int*)pUserData),
                    mCurWorldLayer,
                    worldLayerValid ? 1 : 0,
                    mbLoadZonesDumped ? 1 : 0,
                    mZELs.mUseSize );
                fflush( stdout );
            }
...
#else
                // PHASE 1: Log AddRequest for audit files (one-time per file)
                if(IsAuditFile(spSomeDamnFile) && !SeenAddReq(spSomeDamnFile)) {
                    rReleasePrintf("[AUDIT][AddReq] file=%s msLayer=0x%08X zoneIdx=%d layer=%d\n",
                        spSomeDamnFile, (*(int*)pUserData),
                        ((*(int*)pUserData) & RenderEnums::ZoneMask) >> RenderEnums::ZoneShift,
                        ((*(int*)pUserData) & RenderEnums::LayerOnlyMask));
                }
                // PHASE 2: Set current file for chunk counting
                if(IsAuditFile(spSomeDamnFile)) {
...
END_PROFILE( "RenderManager HandleEvent" );
...
#if defined(RAD_TVOS) && RAD_TVOS_ZONE_PIPELINE_LOG
            {
                ...
                for(int z=0; z<mpZEL->GetNumLoadZones(); z++) {
                    ...
                    if (zn && (strstr(zn, "l1z3") || strstr(zn, "L1Z3"))) foundL1Z3 = true;
                ...
                if (sZoneTriggerCount <= 20 || foundL1Z3 || foundL1Z1) {
                    printf("[TL:SUM] F%u TRIGGER#%d name='%s' pos=(%.0f,%.0f,%.0f) numLoad=%d L1Z3=%d L1Z1=%d\n",
                        gTimelineFrame, sZoneTriggerCount, mpZEL->GetName() ? mpZEL->GetName() : "NULL",
                        trigPos.x, trigPos.y, trigPos.z, mpZEL->GetNumLoadZones(), foundL1Z3, foundL1Z1);
                    fflush(stdout);
                }
                fflush(stdout);
            }
...
BEGIN_PROFILE( "Find Load Zone" );
#ifdef RAD_TVOS
            // Removed verbose preload list - timeline logging now covers this
#endif
            i=-1;
            for(i;i<mpZEL->GetNumLoadZones()&&alreadyLoaded; )
            {
               ...
               if(i<mpZEL->GetNumLoadZones())
               {
...
#if defined(RAD_TVOS) && RAD_TVOS_ZONE_PIPELINE_LOG
                  {
                      ...
                      if (IsTimelineZone(zn)) {
                          const char* tag = GetTimelineTag(zn);
                          printf("[TL:%s] F%u STAGE1_DECISION zoneIdx=%d needsLoad=%d alreadyLoaded=%d\n",
                              tag, gTimelineFrame, i, alreadyLoaded ? 0 : 1, alreadyLoaded ? 1 : 0);
                          fflush(stdout);
                      }
                  }
#endif
...
END_PROFILE( "Find Load Zone" );
            ...
            if(i<mpZEL->GetNumLoadZones())
            {
...
#if defined(RAD_TVOS) && RAD_TVOS_ZONE_PIPELINE_LOG
               {
                   ...
                   if (IsTimelineZone(zoneName)) {
                       const char* tag = GetTimelineTag(zoneName);
                       printf("[TL:%s] F%u STAGE2_FILE_PATH zoneIdx=%d fullPath='%s'\n",
                           tag, gTimelineFrame, i, spSomeDamnFile);
                       fflush(stdout);
                   }
               }
#endif
...
BEGIN_PROFILE( "Add Requests Int" );
                ...
                if(IsAuditFile(spSomeDamnFile) && !SeenAddReq(spSomeDamnFile)) {
                    rReleasePrintf("[AUDIT][AddReq] file=%s msLayer=0x%08X zoneIdx=%d layer=%d\n",
                        spSomeDamnFile, msLayer,
                        (msLayer & RenderEnums::ZoneMask) >> RenderEnums::ZoneShift,
                        (msLayer & RenderEnums::LayerOnlyMask));
                }
                // PHASE 2: Set current file for chunk counting
                if(IsAuditFile(spSomeDamnFile)) {
...
#if defined(RAD_TVOS) && RAD_TVOS_ZONE_PIPELINE_LOG
                {
                    ...
                    if (IsTimelineZone(zoneName)) {
                        const char* tag = GetTimelineTag(zoneName);
                        printf("[TL:%s] F%u STAGE2_ADDREQ_ENQUEUE file='%s' zoneName='%s' zoneIdx=%d msLayer=0x%08X\n",
                            tag, gTimelineFrame, spSomeDamnFile, zoneName, i, msLayer);
                        printf("[TL:%s] F%u STAGE2_ADDREQ_CONTEXT mbDynaLoading_before=%d numLoadZones=%d numDumpZones=%d\n",
                            tag, gTimelineFrame, mbDynaLoading ? 1 : 0, mpZEL->GetNumLoadZones(), mpZEL->GetNumDumpZones());
                        fflush(stdout);
                    }
                }
#endif
