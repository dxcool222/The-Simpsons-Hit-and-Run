...
#include <string.h>  // for strstr in audit logging
...
//==============================================================================
// PHASE 3: Extern declarations for audit counters (defined in RenderManager.cpp)
//==============================================================================
extern bool gAuditDyna;
extern const char* gAuditCurrentFile;
extern int gAuditAddGuts_RoadSeg;
extern int gAuditAddGuts_SpatialNull;
extern int gAuditAddGuts_StateLoad;
extern int gAuditAddGuts_StatePre;
extern int gAuditAddGuts_StateNoLoad;
extern int gAuditAddGuts_StaticEntity;
extern int gAuditAddGuts_StaticPhys;
extern int gAuditAddGuts_Intersect;
//==============================================================================
...
void WorldRenderLayer::AddGuts( IntersectDSG* ipIntersectDSG )
{
   ...
   // PHASE C: track intersect adds for audited files
...
void WorldRenderLayer::AddGuts( StaticEntityDSG* ipStaticEntityDSG )
{
   ...
   // PHASE C: track static entity adds for audited files
   if( gAuditDyna && gAuditCurrentFile )
   {
...
void WorldRenderLayer::AddGuts( StaticPhysDSG* ipStaticPhysDSG )
{
   ...
   // PHASE C: track static phys adds for audited files
   if( gAuditDyna && gAuditCurrentFile )
   {
...
void WorldRenderLayer::AddGuts( RoadSegment* ipRoadSegment )
{
   ...
   // PHASE 3: Track AddGuts for audit files (count, spatial node, state)
   if(gAuditDyna && gAuditCurrentFile) {
      gAuditAddGuts_RoadSeg++;
      if(ipRoadSegment->mpSpatialNode == NULL) gAuditAddGuts_SpatialNull++;
      if(mDynaLoadState == msLoad) gAuditAddGuts_StateLoad++;
      else if(mDynaLoadState == msPreLoads) gAuditAddGuts_StatePre++;
      else gAuditAddGuts_StateNoLoad++;
   }
...
void WorldRenderLayer::DumpAllDynaLoads( unsigned int start, SwapArray<tRefCounted*>& irEntityDeletionList )
{
    ...
    if(mDynaLoadState==msLoad)
    {
        ...
        // PHASE 4: Log dump queued
        if(gAuditDyna) {
            rReleasePrintf("[AUDIT][DumpQueued] state=msLoad start=%u numLoadLists=%d\n", start, mLoadLists.mUseSize);
        }
        mQdDump = true;
...
void WorldRenderLayer::DumpDynaLoad(tName& irGiveItAFuckinName, SwapArray<tRefCounted*>& irEntityDeletionList)
{
   ...
   if (isTimeline) {
       ...
       tUID uid = irGiveItAFuckinName.GetUID();
       radInt64 uid64 = (radInt64)uid;
       unsigned int uidLo = (unsigned int)(uid64 & 0xFFFFFFFFULL);
       unsigned int uidHi = (unsigned int)((uid64 >> 32) & 0xFFFFFFFFULL);
       printf("[TL:%s] F%u STAGE5_DUMP_ENTRY name='%s' UID=0x%08X%08X mLoadLists.size=%d\n",
           tag, gTimelineFrame, dumpName ? dumpName : "NULL", uidHi, uidLo, mLoadLists.mUseSize);
       fflush(stdout);
   }
#endif
   ...
   for(i=0;i<mLoadLists.mUseSize;i++ )
   {
      if( mLoadLists[i]->mGiveItAFuckinName.GetUID() == irGiveItAFuckinName.GetUID() )
      {
         ...
         if (isTimeline) {
             const char* tag = GetTimelineTag(dumpName);
             int entities = mLoadLists[i]->mSEntityElems.mUseSize;
             int roadSegs = mLoadLists[i]->mRoadSegmentElems.mUseSize;
             int staticPhys = mLoadLists[i]->mSPhysElems.mUseSize;
             int intersects = mLoadLists[i]->mIntersectElems.mUseSize;
             printf("[TL:%s] F%u STAGE5_DUMP_FOUND slot=%d entities=%d roadSegs=%d staticPhys=%d intersects=%d\n",
                 tag, gTimelineFrame, i, entities, roadSegs, staticPhys, intersects);
             fflush(stdout);
         }
#endif
         // PHASE 4: Log dump execution with counts (one-liner per zone)
         if(gAuditDyna) {
            const char* zoneName = mLoadLists[i]->mGiveItAFuckinName.GetText();
            bool isL1Target = (zoneName && (strstr(zoneName,"l1r2") || strstr(zoneName,"l1z3") || 
                              strstr(zoneName,"l1r3") || strstr(zoneName,"L1_TERRA") ||
                              strstr(zoneName,"l1_terra")));
            rReleasePrintf("[AUDIT][DumpExec] zone=%s roadSeg=%d static=%d phys=%d intersect=%d isL1Target=%s\n",
                zoneName ? zoneName : "NULL",
                mLoadLists[i]->mRoadSegmentElems.mUseSize,
                mLoadLists[i]->mSEntityElems.mUseSize,
                mLoadLists[i]->mSPhysElems.mUseSize,
                mLoadLists[i]->mIntersectElems.mUseSize,
                isL1Target ? "YES" : "NO");
         }
...
BEGIN_PROFILE( "Cleanup" );
		rReleasePrintf("LoadList Dump: %s Num: %d\n", mLoadLists[i]->mGiveItAFuckinName.GetText(), i );
         ...
         if (isTimeline) {
             const char* tag = GetTimelineTag(dumpName);
             printf("[TL:%s] F%u STAGE5_DUMP_REMOVE slot=%d mLoadLists.size_before=%d\n",
                 tag, gTimelineFrame, i, mLoadLists.mUseSize);
             fflush(stdout);
         }
#endif
         mLoadLists.Remove(i);
         ...
         if (isTimeline) {
             const char* tag = GetTimelineTag(dumpName);
             printf("[TL:%s] F%u STAGE5_DUMP_COMPLETE mLoadLists.size_after=%d zone_removed\n",
                 tag, gTimelineFrame, mLoadLists.mUseSize);
             fflush(stdout);
         }
#endif
END_PROFILE( "Cleanup" );
...
bool WorldRenderLayer::DoPreDynaLoad(tName& irGiveItAFuckinName)//tUID iUID)
{
   ...
   if (isTimeline) {
       ...
       // Get 64-bit UID as two 32-bit parts for accurate logging
       tUID uid = irGiveItAFuckinName.GetUID();
       radInt64 uid64 = (radInt64)uid;
       unsigned int uidLo = (unsigned int)(uid64 & 0xFFFFFFFFULL);
       unsigned int uidHi = (unsigned int)((uid64 >> 32) & 0xFFFFFFFFULL);
       printf("[TL:%s] F%u STAGE1_PRE_ENTRY name='%s' UID=0x%08X%08X mLoadLists.size=%d mDynaLoadState=%d\n",
           tag, gTimelineFrame, checkName ? checkName : "NULL", uidHi, uidLo, mLoadLists.mUseSize, (int)mDynaLoadState);
       // Dump entire mLoadLists contents
       for(int d=0; d<mLoadLists.mUseSize; d++) {
           tUID listUID = mLoadLists[d]->mGiveItAFuckinName.GetUID();
           radInt64 listUID64 = (radInt64)listUID;
           unsigned int lLo = (unsigned int)(listUID64 & 0xFFFFFFFFULL);
           unsigned int lHi = (unsigned int)((listUID64 >> 32) & 0xFFFFFFFFULL);
           printf("[TL:%s] F%u   mLoadLists[%d]='%s' UID=0x%08X%08X\n",
               tag, gTimelineFrame, d, mLoadLists[d]->mGiveItAFuckinName.GetText(), lHi, lLo);
       }
       fflush(stdout);
   }
#endif
   ...
   else
   {
      ...
      if (isTimeline) {
          const char* tag = GetTimelineTag(checkName);
          printf("[TL:%s] F%u STAGE1_PRE_INSERTED slot=%d newSize=%d mDynaLoadState=msLoad\n",
              tag, gTimelineFrame, mCurLoadIndex, mLoadLists.mUseSize);
          // Dump updated mLoadLists
          for(int d=0; d<mLoadLists.mUseSize; d++) {
              printf("[TL:%s] F%u   mLoadLists[%d]='%s'\n",
                  tag, gTimelineFrame, d, mLoadLists[d]->mGiveItAFuckinName.GetText());
          }
          fflush(stdout);
      }
#endif
...
void WorldRenderLayer::DoPostDynaLoad()
{
   ...
   bool isTimeline = IsTimelineZone(loadedName);
   if (isTimeline) {
       const char* tag = GetTimelineTag(loadedName);
       // Count entities committed for this zone
       int entities = 0, roadSegs = 0, staticPhys = 0, intersects = 0;
       if (mCurLoadIndex >= 0 && mCurLoadIndex < mLoadLists.mUseSize) {
           entities = mLoadLists[mCurLoadIndex]->mSEntityElems.mUseSize;
           roadSegs = mLoadLists[mCurLoadIndex]->mRoadSegmentElems.mUseSize;
           staticPhys = mLoadLists[mCurLoadIndex]->mSPhysElems.mUseSize;
           intersects = mLoadLists[mCurLoadIndex]->mIntersectElems.mUseSize;
       }
       printf("[TL:%s] F%u STAGE4_POST_ENTRY name='%s' slot=%d entities=%d roadSegs=%d staticPhys=%d intersects=%d\n",
           tag, gTimelineFrame, loadedName, mCurLoadIndex, entities, roadSegs, staticPhys, intersects);
       printf("[TL:%s] F%u STAGE4_POST_STATE mDynaLoadState changing msLoad->msNoLoad, mCurLoadUID clearing\n",
           tag, gTimelineFrame);
       fflush(stdout);
   }
#endif
   ...
   if(mQdDump)
   {
      ...
      // PHASE 4: Log queued dump execution
      if(gAuditDyna) {
