...
#define INIT_MEM()  Memory::InitializeMemoryUtilities();TvosPlatform::InitializeMemory();
#define SHUTDOWN_MEM()  TvosPlatform::ShutdownMemory();
void MemoryHackCallback() { INIT_MEM() };
#endif // RAD_TVOS
...
#ifdef RAD_TVOS
__attribute__((constructor)) static void srrmemory_tvos_marker()
{
    printf("### SRRMEMORY.CPP LOADED (tvOS) ###\n");
    fflush(stdout);
    os_log(OS_LOG_DEFAULT, "### SRRMEMORY.CPP LOADED (tvOS) ###");
}
#endif
...
void* operator new( size_t size, GameMemoryAllocator allocator )
{
...
#ifdef RAD_TVOS
    // Throttled logging: only log large allocations (>1MB) to reduce spam
    static unsigned int s_allocCount = 0;
    s_allocCount++;
    if( size > 1024 * 1024 )
    {
        os_log(OS_LOG_DEFAULT, "operator new(allocator): LARGE allocator=%d size=%u ptr=0x%llx count=%u", (int)allocator, (unsigned int)size, (unsigned long long)(uintptr_t)pMemory, s_allocCount);
    }
#endif
    if( pMemory == NULL )
    {
        rReleasePrintf( "operator new(allocator) failed: allocator=%d size=%u\n", (int)allocator, (unsigned int)size );
    ...
    if( size > 0 )
    {
        // If this ever happens, something has badly corrupted the allocator.
        // Avoid touching memory so we get a clean backtrace without an extra EXC_BAD_ACCESS.
        if( (uintptr_t)pMemory < 0x100000000ULL )
        {
            os_log(OS_LOG_DEFAULT, "operator new(allocator): suspicious low ptr=0x%llx (size=%u alloc=%d)", (unsigned long long)(uintptr_t)pMemory, (unsigned int)size, (int)allocator);
            __builtin_trap();
        }

        volatile unsigned char* p = reinterpret_cast< volatile unsigned char* >( pMemory );
        unsigned char v0 = p[0];
        p[0] = v0;

        unsigned char vN = p[size - 1];
        p[size - 1] = vN;
    }
#endif
...
HeapStack::HeapStack (GameMemoryAllocator defaultAllocator)
{
    ...
    m_Size = STACKSIZE;
    m_CurPos = 0;
    m_Stack = new(GMA_DEFAULT) GameMemoryAllocator[m_Size];
...
HeapManager* HeapManager::GetInstance ()
{
    ...
    if (!s_Instance)
    {
        ::radThreadCreateLocalStorage (&s_Instance, GMA_DEFAULT);
    ...
    if (!p)
    {
        ...
        p = static_cast<void*>(new(GMA_DEFAULT) HeapManager (currentHeap));  // Create a heap manager.  Init the heap stack with the current heap.
...
#elif defined (RAD_WIN32) || defined( RAD_TVOS ) // these have not been optimized yet.
    #ifdef RAD_RELEASE
    const float HS_DEFAULT = 0.1f;  // For only very core FTech stuff
    #else
    const float HS_DEFAULT = 0.4f;   // For that plus debug comm stuff, etc
