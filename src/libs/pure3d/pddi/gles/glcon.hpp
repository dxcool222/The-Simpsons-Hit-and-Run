...
protected:
    ...
    pglProgram* lightmapProgram[2];   // Lightmap programs (base * lightmap)
...
protected:
    ...
    float* coord;
    float* normal;
    float* uv0;          // Primary UV set (renamed from uv)
    float* uv1;          // Secondary UV set for lightmaps/detail
    unsigned char* colour;
    
    // Computed offsets for vertex attributes
    unsigned coordOffset;
    unsigned normalOffset;
    unsigned uv0Offset;
    unsigned uv1Offset;
    unsigned colourOffset;
    unsigned numUVSets;   // Number of UV sets in this buffer (1 or 2)
