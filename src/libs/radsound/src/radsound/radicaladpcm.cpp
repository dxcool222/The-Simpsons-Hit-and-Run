...
#include "radicaladpcm.hpp"
    ...
    #include <SDL.h>
#endif
static unsigned int s_adpcmDecodeCount = 0;
#endif
...
void DecodeFrames( void* pBufferStart, void * pAdpcmStart, unsigned numAdpcmFrames, unsigned int numChannels )
{
    ...
    bool shouldLog = (s_adpcmDecodeCount <= 20 || (s_adpcmDecodeCount % 200) == 0);
    
    if (shouldLog) {
        unsigned int adpcmFrameSize = numChannels * RADICAL_ADPCM_FRAME_SIZE;
    ...
    if (shouldLog) {
        ...
        unsigned int totalPcmSamples = numAdpcmFrames * RADICAL_ADPCM_SAMPLES_PER_FRAME * numChannels;
        ...
        if (pcmOutput && totalPcmSamples >= 8) {
            ...
            // Check for static-like patterns (all same value or extreme values)
            short minSample = 32767, maxSample = -32768;
            int sampleCount = (totalPcmSamples > 256) ? 256 : totalPcmSamples;
            for (int i = 0; i < sampleCount; i++) {
                if (pcmOutput[i] < minSample) minSample = pcmOutput[i];
                if (pcmOutput[i] > maxSample) maxSample = pcmOutput[i];
            }
