...
#include "clipplayer.hpp"
    ...
    #include <SDL.h>
#endif
static unsigned int s_clipPlayCount = 0;
#endif
...
/* virtual */ void radSoundClipPlayer::Play( void )
{
	...
	)
	{
		...
		if (s_clipPlayCount <= 30 || (s_clipPlayCount % 50) == 0) {
			const char* stateName = (m_State == IRadSoundClipPlayer::Stopped) ? "Stopped" :
			                        (m_State == IRadSoundClipPlayer::Queueing) ? "Queueing" : "QueuedPlay";
			
			IRadSoundHalAudioFormat* fmt = NULL;
			if (m_xRadSoundClip != NULL && m_xRadSoundClip->GetState() == IRadSoundClip::Initialized) {
				IRadSoundHalBuffer* buf = m_xRadSoundClip->GetBuffer();
				if (buf) fmt = buf->GetFormat();
			}
