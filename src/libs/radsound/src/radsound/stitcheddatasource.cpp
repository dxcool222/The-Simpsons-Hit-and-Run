...
#include "stitcheddatasource.hpp"
    ...
    #include <SDL.h>
#endif
static unsigned int s_stitchGetFramesCount = 0;
static unsigned int s_stitchOnLoadCount = 0;
#endif
...
void radSoundStitchedDataSource::OnDataSourceFramesLoaded( unsigned int actuallyRead )
{
    ...
    if (m_xIRadSoundHalAudioFormat != NULL && m_xIRadSoundHalAudioFormat->GetNumberOfChannels() == 1 && actuallyRead > 0) {
        // Calculate start of this chunk's data (before we update m_pCurrentReadPointer)
        char* chunkStart = static_cast<char*>(m_pCurrentReadPointer) - m_xIRadSoundHalAudioFormat->FramesToBytes(actuallyRead);
        // Actually m_pCurrentReadPointer hasn't been updated yet, so it points to start of chunk
        int16_t* samples = (int16_t*)m_pCurrentReadPointer;
        
        // Check if data looks like valid audio or garbage
        int16_t minVal = samples[0], maxVal = samples[0];
        for (int i = 1; i < 8 && i < (int)actuallyRead; i++) {
