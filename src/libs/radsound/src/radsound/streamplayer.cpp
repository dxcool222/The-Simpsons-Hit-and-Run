...
static const unsigned int TVOS_MONO_STREAM_BUFFER_MS = 500;
    ...
    #include <SDL.h>
#endif
static unsigned int s_streamPlayCount = 0;
static unsigned int s_streamDataSourceCount = 0;

static const char* GetStreamEncodingName(IRadSoundHalAudioFormat::Encoding enc) {
    switch(enc) {
...
void radSoundStreamPlayer::Play( void )
{
    ...
    if (s_streamPlayCount <= 30 || (s_streamPlayCount % 50) == 0) {
        const char* stateName = (m_State == IRadSoundStreamPlayer::NoSource) ? "NoSource" :
                                (m_State == IRadSoundStreamPlayer::Queueing) ? "Queueing" :
                                (m_State == IRadSoundStreamPlayer::QueuedPlay) ? "QueuedPlay" :
        ...
        if (m_xIRadSoundHalDataSource != NULL) {
            IRadSoundHalDataSource::State dsState = m_xIRadSoundHalDataSource->GetState();
...
void radSoundStreamPlayer::SetDataSource( IRadSoundHalDataSource * pIRadSoundHalDataSource )
{
    ...
    if (s_streamDataSourceCount <= 50 || (s_streamDataSourceCount % 50) == 0) {
        if (pIRadSoundHalDataSource != NULL) {
            // Try to get format info if data source is initialized
            IRadSoundHalDataSource::State dsState = pIRadSoundHalDataSource->GetState();
